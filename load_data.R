...
#                    "URB", "CNV", "SNO", "BSV", "UNC")
library(rfluxnet)
IGBPnames_006 <- IGBP_006$name
IGBPnames_005 <- IGBP_005$name

metric_spring <- c(
    "TRS1.sos", "TRS2.sos", "TRS5.sos", "TRS6.sos", "DER.sos",
    "UD", "SD", "Greenup", "Maturity"
)
metric_autumn <- c(
    "DER.eos", "TRS6.eos", "TRS5.eos", "TRS2.eos", "TRS1.eos",
...
getPheno_phenofit <- function(fit, INPUT, brks, d, file_pdf, titlestr) {
    ...
    if (IsPlot) {
        ...
        # grid::grid.newpage()
        plot_curvefits(fit, d, titlestr) %>% grid::grid.draw() # plot to check the curve fitting
        dev.off()
    }
    return(fit)
}
...
get_phenofit_GPPobs <- function(sitename,
    ...
    sp <- st[site == sitename, ]
    titlestr <- with(sp, sprintf(
        "[%03d,%s] %s, lat = %5.2f, lon = %6.2f",
