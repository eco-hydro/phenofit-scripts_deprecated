...
df[, c("QC_flag", "w") := qc_summary(SummaryQA, wmin = 0.2, wmid = 0.7)]
df2 = df[, .(ID, t, y = EVI/1e4, w, QC_flag, QC = SummaryQA)]

sp <- read_sf("n:/Research/GEE_repos/gee_whittaker/data-raw/st_test-NorthChina&GuangDong.shp")
...
{
    version = 0.1
    outfile = glue("phenofit_MOD13A2_EVI_{version}.pdf")
